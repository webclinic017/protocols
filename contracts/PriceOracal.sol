...
contract PriceOracle is IPriceOracle {
  ...
  function updateIndexPrice() external override returns (uint224 price){
    ...
    for(uint i=0; i< len; i++)
    {
      ...
      (uint256 reserve0, uint256 reserve1, uint256 blockTimestampLast) = IUniswapV2Pair(indexTokenPair[i]).getReserves();
  ...
  function getPairAddress(address _assetOne, address _assetTwo) external view returns (address) {
   ...
   function getReserves(address tokenA, address tokenB) internal view returns (uint256 reserveA, uint256 reserveB) {
     ...
     */
    function getTokenPrice(address token_address, address token1_address) external view  override returns (uint256) {
        uint256 token_decimals = IERC20Metadata(token_address).decimals();
        uint256 min_amountIn = 1 * 10**token_decimals;
        address _pair = IUniswapV2Factory(uniswapV2Router.factory()).getPair(token_address, token1_address);
        (uint256 reserve0, uint256 reserve1, uint256 blockTimestampLast) = IUniswapV2Pair(_pair).getReserves();
        uint256 price = uniswapV2Router.getAmountOut(min_amountIn, reserve0, reserve1);
        return price;
    }
